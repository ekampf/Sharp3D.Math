...
namespace Sharp3D.Math.Core
{
	...
	public struct ComplexD : ICloneable, ISerializable
	{
		...
		/// Converts the specified string to its <see cref="ComplexD"/> equivalent.
		/// </summary>
		/// <param name="reader">A <see cref="TextReader"/> to read the string representation of a <see cref="ComplexD"/> from.</param>
		/// <returns>A <see cref="ComplexD"/> that represents the vector specified by the <paramref name="s"/> parameter.</returns>
		public static ComplexD Parse(TextReader reader)
		{
			throw new NotImplementedException();
		}
		#endregion
	...
	#region TestComplexD
	/// <summary>
	/// Tests the ComplexD class.
	/// </summary>
	[TestFixture]
	public class TestComplexD
	{
		/// <summary>
		...
		public void Initialization()
		{
			...
			c =  ComplexD.Zero;
			Assert.IsTrue( (c.Real == 0) && (c.Imaginary == 0) );

			c =  ComplexD.One;
			Assert.IsTrue( (c.Real == 1) && (c.Imaginary == 0) );

			c =  ComplexD.I;
			Assert.IsTrue( (c.Real == 0) && (c.Imaginary == 1) );

			c = new ComplexD(0.1, 0.2);
			Assert.IsTrue( (c.Real == 0.1) && (c.Imaginary == 0.2) );

			c = new ComplexD(new ComplexD(0.1, 0.2));
			Assert.IsTrue( (c.Real == 0.1) && (c.Imaginary == 0.2) );
		}
		...
		{
			ComplexD c = new ComplexD(0.1, 0.2);
			Assertion.Assert(ComplexD.ApproxEqual(-c, new ComplexD(-0.1, -0.2)));
		}
		/// <summary>
		...
		public void BinaryOperators()
		{
			...
			c += 1.0;
			Assertion.Assert(ComplexD.ApproxEqual(c, new ComplexD(1.1, 0.2)));

			c -= 1.0;
			Assertion.Assert(ComplexD.ApproxEqual(c, new ComplexD(0.1, 0.2)));

			c = c * 2.0;
			Assertion.Assert(ComplexD.ApproxEqual(c, new ComplexD(0.2, 0.4)));
			...
			c = 1 / c;
			Assertion.Assert(ComplexD.ApproxEqual(c, new ComplexD(0.5, 0.5)));

			// Vector operators
			c = new ComplexD(1,2) + new ComplexD(3,4);
			Assertion.Assert(ComplexD.ApproxEqual(c, new ComplexD(4, 6)));

			c = new ComplexD(1,2) - new ComplexD(3,4);
			Assertion.Assert(ComplexD.ApproxEqual(c, new ComplexD(-2, -2)));
		}
		/// <summary>
		/// Tests conversion.
		/// </summary>
		...
		public void ConversionOperators()
		{
			...
			c = (ComplexD)0.1f;
			Assertion.Assert(ComplexD.ApproxEqual(c, new ComplexD(0.1f, 0)));

			c = (ComplexD)0.1;
			Assertion.Assert(ComplexD.ApproxEqual(c, new ComplexD(0.1, 0)));
		}
		/// <summary>
		/// Tests the modulus method.
		/// </summary>
		[Test]
		public void Modulus()
		{
			ComplexD c = new ComplexD(0.1, 0.2);
			Assertion.Assert(MathFunctions.ApproxEquals(c.GetModulus() * c.GetModulus(), c.GetModulusSquared()));
		}
		/// <summary>
		...
		{
			ComplexD c = new ComplexD(0.1, 0.2).GetConjugate();
			Assertion.AssertEquals(new ComplexD(0.1, -0.2), c);
		}
		...
		public void Normalize()
		{
			ComplexD c = new ComplexD(0.1, 0.2);
			c.Normalize();
			Assertion.Assert(MathFunctions.ApproxEquals(1.0, c.GetModulus()));
		}
	}
	#endregion
}
