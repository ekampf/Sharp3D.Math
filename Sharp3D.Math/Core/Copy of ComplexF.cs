...
namespace Sharp3D.Math.Core
{
	...
	public struct ComplexF : ICloneable, ISerializable
	{
		...
		/// <returns>A <see cref="ComplexF"/> that represents the vector specified by the <paramref name="s"/> parameter.</returns>
		public static ComplexF Parse(TextReader reader)
		{
			throw new NotImplementedException();
		}
		#endregion
	...
	#region TestComplexF
	/// <summary>
	/// Tests the ComplexF class.
	/// </summary>
	[TestFixture]
	public class TestComplexF
	{
		/// <summary>
		...
		public void Initialization()
		{
			...
			c = new ComplexF(0.1f, 0.2f);
			Assert.IsTrue( (c.Real == 0.1f) && (c.Imaginary == 0.2f) );

			c = new ComplexF(new ComplexF(0.1f, 0.2f));
			Assert.IsTrue( (c.Real == 0.1f) && (c.Imaginary == 0.2f) );
		}
		...
		{
			ComplexF c = new ComplexF(0.1f, 0.2f);
			Assertion.Assert(ComplexF.ApproxEqual(-c, new ComplexF(-0.1f, -0.2f)));
		}
		/// <summary>
		...
		public void BinaryOperators()
		{
			...
			c += 1.0f;
			Assertion.Assert(ComplexF.ApproxEqual(c, new ComplexF(1.1f, 0.2f)));

			c -= 1.0f;
			Assertion.Assert(ComplexF.ApproxEqual(c, new ComplexF(0.1f, 0.2f)));

			c = c * 2.0f;
			Assertion.Assert(ComplexF.ApproxEqual(c, new ComplexF(0.2f, 0.4f)));
			...
			c = 1 / c;
			Assertion.Assert(ComplexF.ApproxEqual(c, new ComplexF(0.5f, 0.5f)));

			// Vector operators
			c = new ComplexF(1,2) + new ComplexF(3,4);
			Assertion.Assert(ComplexF.ApproxEqual(c, new ComplexF(4, 6)));

			c = new ComplexF(1,2) - new ComplexF(3,4);
			Assertion.Assert(ComplexF.ApproxEqual(c, new ComplexF(-2, -2)));
		}
		/// <summary>
		/// Tests conversions.
		/// </summary>
		...
		public void ConversionOperators()
		{
			...
			c = (ComplexF)0.1f;
			Assertion.Assert(ComplexF.ApproxEqual(c, new ComplexF(0.1f, 0)));
		...
		{
			ComplexF c = new ComplexF(0.1f, 0.2f).GetConjugate();
			Assertion.AssertEquals(new ComplexF(0.1f, -0.2f), c);
		}
		...
		public void Normalize()
		{
			ComplexF c = new ComplexF(0.1f, 0.2f);
			c.Normalize();
			Assertion.Assert(MathFunctions.ApproxEquals(1.0f, c.GetModulus()));
		}
	}
	#endregion
}
